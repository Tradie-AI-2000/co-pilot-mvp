...
export default function DashboardWidgets({ projects }) {
    ...
    const totalRevenueAtRisk = projects.reduce((acc, proj) => {
        const projectRisk = (proj.hiringSignals || []).reduce((sum, signal) => sum + (signal.revenueAtRisk || 0), 0);
        return acc + projectRisk;
    }, 0);

    const activeProjects = projects.filter(p => p.status === "Active" || p.status === "Construction").length;
    ...
    return (
        <div className="widgets-grid">
            ...
            <div className="widget-card">
                ...
                <div className="widget-content">
                    <span className="widget-label">Revenue at Risk</span>
                    <span className="widget-value">${(totalRevenueAtRisk / 1000).toFixed(1)}k</span>
                    <span className="widget-trend">Weekly margin potential in gaps</span>
                </div>
            </div>
